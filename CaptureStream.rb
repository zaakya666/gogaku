...
#追記
	kouza = YAML.load_file(YMLfilename)
	if $skip_existing && kouza.include?(out_file)
		print "-"
		return true
	end
#
	...
	if file =~ /(.*)\.mp4$/ || file =~ /(.*)\.flv$/
		...
		if $has3args.include?( $audio_extension )
			command = command % [$ffmpeg, file, out_file]
		else
			command = command % [$ffmpeg, file, id3_title, id3_album, '20' + file[0..1], out_file]
		end
...
#use google drive　追記
			...
			File.unlink( out_file ) rescue false
			exit
		end
	end
